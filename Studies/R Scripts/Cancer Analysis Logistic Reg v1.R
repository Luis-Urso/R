#################################################################
# Cancer Analysis using Logistic Regressio                      #
# By Luis A. Urso                                               #
# Objective: Use Logistic Binary Regression to predict cancer #
#################################################################
...
model_cancer <- glm(formula = diagnosis ~ . -compactness_mean -concavity_mean, 
                      data = cancer, 
                      family = "binomial")
...
extract_eq(model_cancer, use_coefs = T,
...
confint(model_cancer, level = 0.95)

###
### MAKING THE CONFUSION MATRIX
###

## Adding the Fitted Values (phat) into the Original Dataset (cancer)

cancer$phat <- model_cancer$fitted.values
...
confusionMatrix(table(predict(model_cancer, type = "response") >= cutoff_factor,
                      cancer$diagnosis == 1)[2:1, 2:1])

## Visualizing the Confusion Matrix Metrics

data.frame(Recall = confusionMatrix(table(predict(model_cancer,
                                                         type = "response") >= cutoff_factor,
                                                 cancer$diagnosis == 1)[2:1, 2:1])[["byClass"]][["Sensitivity"]],
           Specif = confusionMatrix(table(predict(model_cancer,
                                                          type = "response") >= cutoff_factor,
                                                 cancer$diagnosis == 1)[2:1, 2:1])[["byClass"]][["Specificity"]],
           Acc = confusionMatrix(table(predict(model_cancer,
                                                    type = "response") >= cutoff_factor,
                                                 cancer$diagnosis == 1)[2:1, 2:1])[["overall"]][["Accuracy"]]) %>%
  kable() %>%
...
predictions <- prediction(predictions = model_cancer$fitted.values, 
                        labels = as.factor(cancer$diagnosis))
...
ROC <- roc(response = cancer$diagnosis, 
           predictor = model_cancer$fitted.values)
